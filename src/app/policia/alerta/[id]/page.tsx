...
import { ArrowLeft, AlertTriangle, Link2, MapPin, Smartphone, User, Clock, TrendingUp } from 'lucide-react';
import FaseGrooming from '@/components/viz/FaseGrooming';
import ActivityHeatmap from '@/components/viz/ActivityHeatmap';
import { alertRows, linguisticPatterns, temporalEvents } from '@/lib/mock-data';
...
export default function AlertaDetailPage() {
  ...
  const alert = alertRows.find((r) => r.id === id) ?? alertRows[0];
  const urgencyColor = urgencyColors[alert.urgency] ?? '#999';
    ...
    <div style={{ padding: isMobile ? '24px 16px' : '32px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          {alert.linked && (
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 12px', borderRadius: 999, background: 'var(--color-terra-50)', border: '1px solid var(--color-terra-200)', fontSize: '0.76rem', fontWeight: 500, color: 'var(--color-terra-600)', fontFamily: 'var(--font-body)' }}>
              <Link2 size={12} /> Vinculado a reporte directo
            </span>
          )}
        </div>
      ...
      {/* Vinculación callout — only for linked alerts */}
      {alert.linked && (
        <motion.div
          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
          style={{
            marginBottom: 20, padding: '14px 18px', borderRadius: 12,
            background: 'var(--color-terra-50)', border: '1.5px solid var(--color-terra-300)',
            display: 'flex', alignItems: 'flex-start', gap: 12,
          }}
        >
          <AlertTriangle size={16} color="var(--color-terra-500)" style={{ flexShrink: 0, marginTop: 2 }} />
          <div>
            <div style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.85rem', color: 'var(--color-terra-700)', marginBottom: 3 }}>
              Vinculación detectada
            </div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', color: 'var(--color-terra-600)', lineHeight: 1.5 }}>
              Este caso está vinculado a un reporte directo enviado por canal ANCLA. La víctima identificada en esta alerta corresponde al folio REP-001 recibido hace 1h.
            </div>
          </div>
        </motion.div>
      )}
      ...
      >
        {[
          { icon: MapPin, label: alert.zone },
          { icon: User, label: alert.age },
          { icon: Smartphone, label: alert.platform },
          { icon: Clock, label: formatTimestamp(alert.timestamp) },
        ].map(({ icon: Icon, label }) => (
      ...
      {/* Two-column layout: fase + linguistic / heatmap */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        >
          ...
          <FaseGrooming activePhase={alert.groomingPhase} />
        </motion.div>
            ...
            {linguisticPatterns.map((p) => (
              <div key={p.pattern}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.8rem', fontWeight: 500, color: 'var(--color-text-primary)' }}>
                    {p.pattern}
                  </span>
            ...
            {temporalEvents.map((ev, i) => (
              ...
              >
                ...
                <div style={{
                  ...
                  background: eventTypeColors[ev.type] ?? '#999',
                  border: '2px solid white',
                  boxShadow: `0 0 0 2px ${eventTypeColors[ev.type] ?? '#999'}40`,
                }} />
                <div>
